...
a << "Roger" # .push "Roger" would also work
p a
...
h[ 3 ] = "Three" # For super nerds only: h.merge {3 => "Three"}
...
users["Roger"] = { :twitter => "rogermellie", :favorite_numbers => [12, 42, 666] }
...
p users["Anil"][:favorite_numbers].select { |n| n.even? } # .select(&:even?)
# 1. How would you return an array of the favorite numbers common to all users?
users["Jonathan"][:favorite_numbers] & users["Erik"][:favorite_numbers] & users["Anil"][:favorite_numbers] & users["Roger"][:favorite_numbers]
# Fancy version 1: users.map { |name, user| user[:favorite_numbers] }.inject(&:&)
# Fancy version Tim: users.map { |name, user| user[:favorite_numbers]}.inject{ |memo, a| memo & a }
